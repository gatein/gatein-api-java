...
/**
 * A node object which represents the current state of a node retrieved from the portal. All changes to the node
 * are not saved until {@link Navigation#saveNode(Node)} is called.
 *
...
public interface Node extends Displayable, Iterable<Node>, Serializable
{
    ...
    * @throws EntityAlreadyExistsException if a child of the same name already exists.
    */
   Node addChild(int index, String childName) throws IllegalArgumentException, IllegalStateException, IndexOutOfBoundsException, EntityAlreadyExistsException;
    ...
    * <p>
    *    For example if a node has 3 visible and one hidden children nodes and a filter is applied to only accept
    *    visible nodes, getChildrenCount would only return 2, iterating over the filtered node would not include
    *    the hidden node, etc.
    * </p>
